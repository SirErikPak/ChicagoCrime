truncate table chicago.community_areas;
insert into chicago.community_areas
select 	the_geom, 
		area_number, 
		community, 
		replace(shape_area, ',', '')::double precision,
   		replace(shape_len, ',', '')::double precision
from chicago.staging_community_areas;
ANALYZE  chicago.community_areas;

-------------------------------------
truncate table chicago.neighborhoods;
...
delete from chicago.staging_police_beats
where district in (31)
-----

truncate table chicago.police_beats;
insert into chicago.police_beats
select the_geom, district, sector, beat_num
	from chicago.staging_police_beats;
ANALYZE  chicago.police_beats;
--------------------------------

truncate table chicago.zip_codes;
