...
def find_missing_crime_months(data_df: pd.DataFrame, 
    ...
    d = _prepare_era_data(data_df, era_label)
...
def _prepare_era_data(data_df: pd.DataFrame, era_label: str) -> pd.DataFrame:
    """
...
def fill_missing(data_df: pd.DataFrame, era_label: str) -> pd.DataFrame:
    """
    ...
    """
    # 0. Call Helper for Data Preparation: Standardize dates and assign era metadata
    data_df = _prepare_era_data(data_df, era_label)
